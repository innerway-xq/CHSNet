...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        q = self.wq(x).reshape(B, Nx, self.num_heads, C //
                               self.num_heads).permute(0, 2, 1, 3)
        # BNyC -> BNyH(C/H) -> BHNy(C/H)
        k = self.wk(y).reshape(B, Ny, self.num_heads, C //
                               self.num_heads).permute(0, 2, 1, 3)
        # BNyC -> BNyH(C/H) -> BHNy(C/H)
        v = self.wv(y).reshape(B, Ny, self.num_heads, C //
                               self.num_heads).permute(0, 2, 1, 3)

        # BHNx(C/H) @ BH(C/H)Ny -> BHNxNy
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # (BHNxNy @ BHNy(C/H)) -> BHNx(C/H) -> BNxH(C/H) -> BNxC
